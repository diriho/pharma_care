...
The authentication provider verifies the user’s credentials.
A secure authenticated session is established.
The application identifies the user’s role.
The user is granted access only to authorized patient functionality.
Protected backend endpoints independently verify the authenticated session.
...
The staff member initiates dispensing.
The application requests re-authentication.
The authentication mechanism verifies the staff member.
The system records the authorization event.
Dispensing can proceed only if authentication succeeds.
